...
`workflow/bundle.py`, `context.py`, `names.py`, `select.py`, `setops.py`, and
...
1. **`workflow/steps/merge.py:26`** — `is_done` returns `False`, but its input is
   `bundle.evaluated(ctx)`, which `archive` has already moved out of the bundle
   directory. Any failure after `archive` and before `advance` makes every retry
   crash rather than resume, and the bundle becomes permanently un-completable.
   ...
   .../p2/eval/<bundle_name>`. The comment at merge.py:24 argues `False` is
   "cheap and correct" because the done-set is shared — right about the *output*, but it
...
## Naming assumptions that don't match `bundle.begin`

3. **`workflow/bundle.py:69`** — `source()` passes `ctx.bundle_name` to
   `Path.glob` as a literal pattern, so it only matches a file named exactly the
   bundle name. But `bundle.begin` resolves the queued file with `stem:` (prefix)
   matching, so `wf eval p1 a` legitimately creates bundle dir `a/` holding
   `a.pairs`, and every later `complete p1 a` step fails with
   `ValueError: no a in .../p1/eval/a`.
   Reproduced. `workflow/steps/p1_archive.py:23` (`outputs`) shares the
   assumption — it renders the archived name as `done/in/<bundle_name>` while
   `run_step` actually moves `source.name`, so `is_done` is wrong in the same case.

4. **`workflow/steps/p1_advance.py:22`** — `inputs`/`outputs`/`run_step` glob
   `f"{ctx.bundle_name}*.p1.{kind}"`, but `p1_extract.produced_bundle_name`
   renders the names from the `.jsonl` *result* stem, which nothing enforces to
   be prefixed by the
   current bundle name (`bundle.begin` only asserts the prefix for the queued
   input). If evalpair writes e.g. `results.jsonl` into the bundle, extract
   produces `results.90.10.p1.*`, advance moves nothing, and `bundle.finish` then
   raises `bundle <bundle_name> still holds: ...` — after archive has already
   run, i.e. straight into the non-resumable state of finding 1.
...
5. **`workflow/steps/p2_retrieve.py:80`** — `staging.rename(enex_dir(ctx))` fails
   ...
   `complete p2` on a bundle whose notes were already retrieved raises
...
6. **`workflow/steps/p1_archive.py:33`** — not restartable mid-step: it moves the
   ...
   <bundle dir>` from `bundle.one`, because the result has already left.
...
7. **`workflow/steps/p2_archive.py:35`** — same partial-failure hole, plus a force
   ...
   `--force`; renaming onto an existing non-empty `enex/<bundle_name>` raises
...
8. **`workflow/steps/filter.py:24`** — `outputs` is `[ctx.dest]` under the default
   ...
   outside any bundle. `wf extract p1 yes all -o out.pairs --pm 0.5` after an
...
9. **`src/filter.py:78`** — the warn-and-continue around `iter_projected_blocks`
   ...
   `WARNING: skipping ...`, writes an empty `<bundle_name>.p1.yes` into
   `p2/queued`, logs `Filtered 0 pairs`, and exits 0, where the old single-path
   code propagated the
   error.
