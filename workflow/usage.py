...
def make_global_parser() -> argparse.ArgumentParser:
    ...
    # Honoured where a command shells out to a long-running producer, and
    # refused by name everywhere in `best` that would otherwise ignore it: a
    # flag that silently does nothing on a command that writes state is worse
    # than one that is not accepted there.
    p.add_argument("--dry-run", action="store_true", dest="dry_run",
                   help="print the search command instead of running it "
                        "(best gen dfs.seed|dfs.best, best prepare)")
