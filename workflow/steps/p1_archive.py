# steps/p1_archive.py
#
# Move the batch's *inputs* into done/{in,out}. The artifacts this batch
...
# evalpair's, not ours, and the batch directory is already the namespace.
MOVES = (("*.jsonl", "out"), (batch.INPUT_GLOB["p1"], "in"))


def inputs(ctx) -> list[Path]:
    return [p for glob, _ in MOVES for p in sorted(ctx.batch_dir.glob(glob))]


def outputs(ctx) -> list[Path]:
    return [_done(ctx, inout) / p.name
            for glob, inout in MOVES for p in sorted(ctx.batch_dir.glob(glob))]


def is_done(ctx) -> bool:
    # This step's whole job is to empty the batch of its inputs, so their
...
def run_step(ctx) -> None:
    ...
    for glob, inout in MOVES:
        for found in sorted(ctx.batch_dir.glob(glob)):
